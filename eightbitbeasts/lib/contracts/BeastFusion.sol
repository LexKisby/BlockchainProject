...
contract BeastFusion is BeastGenerator {
    ...
    mapping(uint => Extract) extractToTamer; 


    modifier onlyOwner(uint _beastId) {
        require(msg.sender == beastToTamer[_beastId], "You do not own this beast");
        _;
    }

    //for existing beasts only
    function _triggerRecoveryPeriod(uint _beastId, uint32 _factor) internal {
        beasts[_beastId].readyTime = uint32(block.timestamp + _factor * minRecoveryPeriod);
    }  

    function _isReady(uint _beastId) internal view returns (bool) {
        return (beasts[_beastId].readyTime <= block.timestamp);
    }

    function _isSlime(uint _beastId) internal view returns (bool) {
        return (beasts[_beastId].dna[0] == 0);
    }

    //######################################################################
    //main reproduction functions
    function _beastFusion(uint _primaryId, uint _secondaryId, string memory _name) internal returns (bool) {
          //create new data
        ...
        uint8[2] memory newSpecies = _calculateSpecies(_primaryId, _secondaryId, 1);
    ...
    function _slimeFusion(uint _primaryId, uint _secondaryId,string memory _name) internal returns (bool) {
         //create new data
        Stats memory newStats = _calculateStats(_primaryId, _secondaryId, 2);
        uint8[22] memory newDna = _calculateDna(_primaryId, _secondaryId);
        uint8[2] memory newSpecies = _calculateSpecies(_primaryId, _secondaryId, 4);
        newDna[0] = newSpecies[0];
        newDna[1] = newSpecies[1];
        //Check dna
        uint attempts = 1;
    ...
    function _slimePrimaryFusion(uint _primaryId, uint _secondaryId, string memory _name) internal returns (bool) {
         //create new data
        Stats memory newStats = _calculateStats(_primaryId, _secondaryId, 1);
        uint8[22] memory newDna = _calculateDna(_primaryId, _secondaryId);
        uint8[2] memory newSpecies = _calculateSpecies(_primaryId, _secondaryId, 2);
        newDna[0] = newSpecies[0];
        newDna[1] = newSpecies[1];
        //Check dna
        uint attempts = 1;
    ...
    function _slimeSecondaryFusion(uint _primaryId, uint _secondaryId, string memory _name) internal returns (bool) {
        //create new data
        Stats memory newStats = _calculateStats(_primaryId, _secondaryId, 1);
        uint8[22] memory newDna = _calculateDna(_primaryId, _secondaryId);
        uint8[2] memory newSpecies = _calculateSpecies(_primaryId, _secondaryId, 3);
        newDna[0] = newSpecies[0];
        newDna[1] = newSpecies[1];
        //Check dna
        uint attempts = 1;
    ...
    //assister functions#####################################################
    function _calculateStats(uint _id1, uint _id2, uint _type) internal view returns (Stats memory) {
        //normal calculation
        if (_type == 1) {
            Stats memory newStats = Stats(
                _weightedAvg(beasts[_id1].stats.hp, beasts[_id2].stats.hp),
                _weightedAvg(beasts[_id1].stats.attackSpeed, beasts[_id2].stats.attackSpeed),
                _weightedAvg(beasts[_id1].stats.evasion, beasts[_id2].stats.evasion),
                _weightedAvg(beasts[_id1].stats.primaryDamage, beasts[_id2].stats.primaryDamage),
                _weightedAvg(beasts[_id1].stats.secondaryDamage, beasts[_id2].stats.secondaryDamage),
                _weightedAvg(beasts[_id1].stats.resistance, beasts[_id2].stats.resistance),
                _weightedAvg(beasts[_id1].stats.accuracy, beasts[_id2].stats.accuracy),
                _weightedAvg(beasts[_id1].stats.constitution, beasts[_id2].stats.constitution),
                _weightedAvg(beasts[_id1].stats.intelligence, beasts[_id2].stats.intelligence)
            );
            return newStats;
        }
        //slimes calculation
        if (_type == 2) {
            Stats memory newStats = Stats(
                ...
                beasts[_id1].stats.attackSpeed.add(beasts[_id2].stats.attackSpeed),
                beasts[_id1].stats.evasion.add(beasts[_id2].stats.evasion),
                beasts[_id1].stats.primaryDamage.add(beasts[_id2].stats.primaryDamage),
                beasts[_id1].stats.secondaryDamage.add(beasts[_id2].stats.secondaryDamage),
                beasts[_id1].stats.resistance.add(beasts[_id2].stats.resistance),
                beasts[_id1].stats.accuracy.add(beasts[_id2].stats.accuracy),
                beasts[_id1].stats.constitution.add(beasts[_id2].stats.constitution),
                beasts[_id1].stats.intelligence.add(beasts[_id2].stats.intelligence)
            );
            return newStats;
        }
    ...
    function _weightedAvg(uint16 a, uint16 b) internal pure returns (uint16) {
        uint result = 2 * uint(a);
        result = (result + b) / 3;
        return uint16(result);
    }
    function _calculateSpecies(uint _primaryId, uint _secondaryId, uint _type) internal view returns (uint8[2] memory) {
        ...
        if (_type == 1) {
            ...
            } else {result[1] = beasts[_secondaryId].dna[1];}
            return result;
        }
        //slime primary calc
        //always returns 00
        ...
        if (_type == 4) {
            ...
            if (beasts[_primaryId].dna[0] == 0 && beasts[_primaryId].dna[1] == 0) {
    ...
    function _calculateDna(uint _id1, uint _id2) internal view returns (uint8[22] memory) {
        uint8[51] memory p = _generateDigits(uint256(keccak256(abi.encodePacked(block.timestamp))));
        uint8[22] memory dna; 
        for (uint i = 2; i < 22; i = i + 2) {
            ...
            if (p[i+1] > 6) {
                ...
                //else take dominant allele
            } else {dna[i+1] = beasts[_id2].dna[i];}
        }
    ...
    function BeastFusionSwitch(uint _primaryId, uint _secondaryId, string calldata _name) external onlyOwner(_primaryId) returns (bool) {
        ...
        if (ownsSecondary != true) {
            ownsExtract = (extractToTamer[_secondaryId].owner == msg.sender && extractToTamer[_secondaryId].expiry > block.timestamp);
            if (ownsExtract != true) {
        ...
        //neither are slime ##############################################
        else {success = _beastFusion(_primaryId, _secondaryId, _name);}
